...
import { PublicKey, SystemProgram, Keypair, GetAccountInfoConfig } from "@solana/web3.js";
...
async function main() {
    ...
    const roundInfoData: any = await program.account.roundInfo.fetch(roundInfoAccount);
    console.log("totalDeps: ", roundInfoData.totalDeposits.toString());
    console.log("deposits: ", roundInfoData.deposits.map((deposit: any) => deposit));
    console.log("deposit_indices: ", roundInfoData.depositIndices.map((index: any) => index));
    ...
    // console.log("TX: ", await program.methods.drawWinner(vaultBump, new PublicKey("E3QrfXGAdTdTF5RGa3uWGE3Yn2BejuyAFSbZVkBBsZHA"))
    //     .accounts({
